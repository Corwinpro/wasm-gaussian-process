...
const drawCurve = (points, tension, colour) => {
    ctx.beginPath();
...
const drawGP = () => {
    ...
    for (var i = 0; i < 20; i++) {
        ...
        const points = Array.from(samples, (value, index) => ({ x: sampleInputs[index] * 250, y: value * 25 + 300 }));
        const randomColor = "#" + Math.floor(Math.random() * 16777215).toString(16);

        drawCurve(points, 1.0, randomColor);
    }
